...
class UFormer(nn.Module):
    def __init__(
        ...
        self.output_proj = OutputProjection(in_channel=2*embed_dim, out_channel=1) #TODO channel
    ...
    def forward(self, x, mask=None):
        ...
        # y[0, 0, :, :] += x[0, 0, :, :] # if we want to skip link img transformer's reconstruction to the output of combo network
        # averaged_input = torch.mean(x, dim=1, keepdim=True)
        # y[:, 0, :, :] += (averaged_input[:, 0, :, :]) # if we skip link average of img transformer + freq transformer reconstructions to output of combo network
        return y 
