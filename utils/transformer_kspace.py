...
class UFormer(nn.Module):
    def __init__(
        self, img_size=128, in_channels=2, embed_dim=4,
        ...
        """
        len(depth) = 5, i.e. 2 encoder stages, 1 bottleneck, 2 decoder stage for now.
        ...
        self.output_proj = OutputProjection(in_channel=2*embed_dim, out_channel=2) # Changed from in_channels
    ...
    def forward(self, x, mask=None):
        ...
        y[0, 0, :, :] += x[0, 4, :, :] # add real channel of 3rd kspace to output's first channel
        y[0, 1, :, :] += x[0, 5, :, :] # add imaginary channel of 3rd kspace to output's second channel
        return y 
    
s = nn.Sigmoid()
#takes freq domain input and convert to img domain output
#try to use the absolute 
def model_out_to_img(out):
        # Given freq data of input shape 2x128x128, return one real output image 1x128x128
        complex_tensor = torch.complex(out[:,0],out[:,1])
        img_data = torch.absolute(torch.fft.ifft2(complex_tensor))
        # img_data = torch.multiply(s(img_data), 255) # we did this back when data from hard drive was not normalized to begin with
        #I still think we should use this as there's no reason not to normalize and it should help with accuracy. I tested normalizing the data in the beginning steps (both freq and img domains) and the model had better accuracy with having this here. Although the ***input*** data is normalized the model still interpolates beyond what the input is and produces values above 255. By normalizing here we remove that noise and prevent any gradients from being passed on for weight updates when the ifft output is above the values corresponding to a pixel value of below 0 or above 1. - Mati 
        return img_data
    

def model_out_to_imgt(out):
    #output = torch.squeeze(out) # frequency domain output

    complex_tensor = torch.complex(output[0,:,:],output[1,:,:])
    #img_data = torch.fft.ifft2(complex_tensor).real
    img_data = torch.fft.ifft2(complex_tensor).abs()
    img_data = torch.multiply(s(img_data), 255)
